...
# --- VSMS палитра культур (app-scope) ---
# Цвет культуры выбирается из этой курируемой палитры (swatch-пикер в форме культуры,
# справочник «Культуры»). Любой рендер (чип ленты, подложка строки, заливка heatmap
# «Сводки», серия графика аналитики, печатный лист) берёт Culture.color — НЕ хардкодит.
# Culture.color остаётся hex-строкой (миграции нет): палитра задаёт допустимые значения
# и UI выбора. Единственное определение в коде — lib/culture-palette.ts (CULTURE_PALETTE);
# эти hex — зеркало. Подобрана так, чтобы 6–8 культур различались рядом, работали заливкой
# heatmap на трёх уровнях прозрачности, читались столбцом графика и не сливались в grayscale
# на печати. Держится в стороне от статусных цветов отгрузок (status-fills: азурный/янтарный/
# мятный). Off-palette цвета старых культур допустимы (fallback «свой цвет» в форме), но новые
# культуры выбирают цвет только из палитры.
culture-palette:
