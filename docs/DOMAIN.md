...
`movement_type`: return (тара фермер→завод с продукцией, **двухфазно через транзит**, из Shipment) · delivery (тара завод→фермер, **двухфазно через транзит**, из MaterialShipment) · transfer (фермер→фермер) · scrap (годная→лом, same loc) · disposal (лом→out, утиль) · adjustment (ручная правка) · **opening (начальный остаток: from=null→локация, source=manual; задаётся один раз, редактируется заменой)**.
...
3. **Движение тары — ДВУХФАЗНОЕ, привязано к статусам** (дебет при отправке, кредит при прибытии; между — транзит). Количество тары = ceil(planned_weight / PackagingNorm.avg_unit_weight по тройке фермер×культура×тип); навал (тип=null) → движения нет; нет нормы → переход заблокирован. capacity_kg не участвует.
   ...
   - **Доставка (MaterialShipment, тара завод→фермер):** отдельный рейс (свой водитель/даты, груз=тара, кол-во задаётся в MaterialShipmentItem.quantity, не из веса). `planned→sent` → `завод → -2`; `sent→arrived` → `-2 → фермер`. Откат сторнирует плечо. С входящим рейсом НЕ связан.
   - **Начальный остаток (opening):** разовое движение `from=null → локация` (завод и фермеры по типам), `movement_type=opening`, `source=manual`. Редактирование — заменой opening-движения (это setup-данные, не операционный леджер).
