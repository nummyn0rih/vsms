...
- `accepted_weight_kg` — производное: **simple** = actual × (1 − brak%); **calibre** = actual × (1 − brak%) × Σ(% принятых категорий). Брак (порча) — поле акта у ВСЕХ культур.
...
| BR-10 | `brak_percent` (порча, % от actual) — поле акта у **ВСЕХ** культур (simple и calibre), НЕ категория. Среди категорий схемы калибровки категории «брак» **НЕТ**. **Calibre — ДВУХСТУПЕНЧАТО (Вариант 2):** (1) годное = `actual × (1 − brak%)`; (2) раскладка ГОДНОГО по категориям схемы — **размерным ИЛИ безразмерным** (напр. 6–9 см, >12 см, или «Стандарт»/«Пульпа» для томата), Σ % категорий = 100% ОТ ГОДНОГО (не от actual). `accepted = actual × (1 − brak%) × Σ(% категорий is_accepted=true)`. Неприёмные категории (`is_accepted=false`, напр. >12 см) — не в зачёт, это НЕ брак (годны по сохранности). Дефолт привязки категории к строке: `is_accepted=true` → наследует `ShipmentItem.contract_line_id`; `is_accepted=false` → null (нестандарт базово без оплаты; админ может вручную привязать к строке, напр. «нестандарт >12»). |
...
| Что | Формула | База |
|---|---|---|
| accepted_weight | simple: actual×(1−brak%) · calibre: actual×(1−brak%)×Σ(принятых категорий %) | принятый |
...
**Для calibre-культур** (двухступенчато: брак% → калибровка ГОДНОГО по размерным категориям, каждая со своей привязкой к строке). Обозначим `годное = actual × (1 − brak%)`:

| Что | Формула | База |
|---|---|---|
| выполнение строки | Σ (годное × % категорий, привязанных к ЭТОЙ строке) / volume_tons | принятый |
| стоимость | Σ по категориям (годное × % × price_per_kg ИХ строки) | принятый |
| accepted_weight позиции | actual × (1 − brak%) × Σ(% принятых категорий) | принятый |
| нестандарт | Σ (годное × % категорий is_accepted=false с размерами) | принятый-сорт |
| брак (порча) | actual × brak% | фактический |

- **Брак (порча)** — `brak_percent`, % от actual; поле акта у ВСЕХ культур (simple и calibre). Σ% категорий схемы (размерных ИЛИ безразмерных) считается ОТ ГОДНОГО (после вычета брака), не от actual.
