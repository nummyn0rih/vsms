...
- [~] **Экспорт в Excel** (SheetJS введён) — В РАБОТЕ.
  - [x] Отгрузки + Приёмка (клиентский SheetJS по фильтрам, общий `lib/xlsx-export`). `PROMPTS-EXPORT-XLSX.md`. Коммиты `a27319c` + `0f704e7` (кнопка в row 1).
  - [~] Сводка + План (культура×день из `getPlanWeek`) — ТЕКУЩИЙ. `PROMPTS-EXPORT-XLSX-2.md`.
  - [ ] (опц.) Аналитика — таблицы KPI/выполнения (бэклог).
