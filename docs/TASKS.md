...
**Текущий фокус:** Этап B (Ядро отгрузок). Закрыты: B-контракты-мини, B1, B2, B2.5; B3 срезы 17a/17b — приняты. В работе B3: (1) заменить временные `AlertDialog` статусов на свёрстанные диалоги (`prototypes/status-dialogs-*`), (2) срез 17c — тулбар (фильтры/поиск/неделя/Excel). Перед 17c — разбор остаточных недоработок после fix-1/fix-2.
