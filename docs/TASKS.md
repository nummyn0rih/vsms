...
версия v1** (responsive) — ЗАКРЫТА. **Мобильный v2** (Сводка + План, read-only) — ЗАКРЫТ. **Текущий срез — печатные формы** (лаконичные табличные
версии видов: Отгрузки · Сводка · План · Приёмка · Аналитика; print-CSS, без графики). Далее: сравнение сезонов
(после истории, этап G) · бэклог V1.1-фильтров. Промпты — `docs/prompts/PROMPTS-*.md`.
...
- [~] Печатные формы (ТЕКУЩИЙ): рабочие виды (Отгрузки · Сводка · План · Приёмка) — лаконичные A4-таблицы БЕЗ графики; **Аналитика — с графиками** (print-версия дашборда, KPI + 4 графика). Подход: print-CSS, отдельные print-представления, контекст (сезон/неделя/фильтры), «Печать»→PDF. Дизайн-спека `DESIGN-PRINT.md`. (Печать акта позиции — бэклог, `DESIGN-ACT-PRINT.md` замещена.)
