...
- [x] **C1c — баги/косметика + лента по позиции:** диалог акта поднят на уровень доски (`AcceptanceBoard`) — переживает перетасовку зон при `markArrived` (шорткат «Акт» из зоны 1 теперь открывает форму). Единый статус-бейдж машины (взвешено → «Частично принята», без дубля/overflow). После `saveAct` — реактивный refresh доски и ленты. Принятая позиция: поле веса read-only/затемнено (правка — через акт). Калибр: опция «— не в зачёт» (сброс привязки в null), кг слева от % однострочно. Лента: статус приёмки **по позиции** (BR-13) — «факт {вес} кг» / принятая «факт · к оплате {принятый} кг», независимо от соседей, округление до целых; «к оплате» из точных % (`computeAcceptedKg`). № акта в рамках сезона (BR-9): хранится `{season_year}-{введённое}`, коллизия → «занят в этом сезоне» (без миграции — глобальный @unique эквивалентен паре сезон+номер).
- [x] **C1d — одноступенчатая калибр-модель (BR-10) + вёрстка:** калибр-форма переведена на одноступенчатую модель — «Чистый вес»/двухступенчатость убраны; `Σ(% категорий) + brak% = 100% от факта`, `accepted = actual × Σ(принятых %)`. Брак — поле акта, рендерится **последней строкой** таблицы категорий (ред. `%`, «Строка контракта» = нередакт. «— не в зачёт»). Факт на одной строке с № акта; заголовки колонок `Категория · кг · % · Строка контракта` над данными; лейблы «Категории»/«% от факта». `computeAcceptedKg` (calibre) и Σ-валидация `saveAct` исправлены (брак входит в 100%, не вычитается дважды). simple-путь без изменений. Вёрстка ленты/доски: бейдж позиции «Акт принят» (иконка-документ), бейджи машины «взвешено · N/M» / «принято · N/M» в одну строку, колонки ленты сдвинуты («к оплате» не наезжает), read-only вес и инпут на одной линии (`h-8`). БЕЗ склада (C2)/стоимости (C3).
