...
**Текущий фокус:** A–E, D2-ops, D3-2, transfer, **B5 (доска-планировщик)**, shipment-revert — ЗАКРЫТЫ.
Контуры поставки/тары/ингредиентов/планирования/переноса/откатов замкнуты. **Следующий кандидат:**
acceptance auto-date (приёмка: авто-дата + диалог + защита от задним числом) ИЛИ широкий V1.1
(Сводка/Heatmap, AlertRule-алерты, карточка поставщика, фильтры). Промпты — `docs/prompts/PROMPTS-*.md`.
...
- [ ] acceptance auto-date — приёмка: авто-дата прибытия + диалог «датой отгрузки / сегодня» + защита от задним числом.
- [ ] широкий V1.1 — Сводка/Heatmap наполнение, AlertRule-алерты, карточка поставщика, фильтры.
