...
V1.1 ЗАКРЫТ. Идёт **этап F/V2**: аналитический дашборд (Recharts) ✓, факт. средний вес рейса ✓. **Текущий срез —
мобильная версия v1** (Лента-просмотр · Приёмка · Контакты водителей; responsive-first, карточный список).
Далее F/V2: печатные формы (PDF) · сравнение сезонов (после истории, этап G). Промпты — `docs/prompts/PROMPTS-*.md`.
