...
**Текущий фокус:** Этап B (Ядро отгрузок). Закрыты: B1, B2, B2.5, B3 (лента + диалоги), **B4a** (вид «План»: цели день/неделя, конверсия, прогресс) + **B4c** (состав недели `WeeklyPlanScope`). Хвосты: chore-коммит — удалить осиротевшие `previewShipmentTare`/`ShipmentTarePreview`. **B4b (перевеска: вкладка «Приёмка», `actual_weight`, sent→arrived, бар коалесит факт) — закрыт.** Следующий срез — **B5 (доска dnd на вкладке «План»)**.
