...
версия v1** (responsive) — ЗАКРЫТА (шелл+лента · приёмка · водители). **Текущий блок — мобильный v2: План/Сводка**
(read-only на телефоне, карточный список по культурам, источник `getPlanWeek`). Далее: печатные формы (PDF) ·
сравнение сезонов (после истории, этап G) · бэклог V1.1-фильтров. Промпты — `docs/prompts/PROMPTS-*.md`.
...
**Мобильная версия v1 ЗАКРЫТА** (3/3). Responsive-паттерн (шелл + `md:`-варианты + карточные списки) — эталон для будущих мобильных экранов. Осталось (v2): мобильные План/Сводка, нижний-vs-drawer доводки.
