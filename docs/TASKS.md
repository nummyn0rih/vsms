...
Сводка ✓ → AlertRule-алерты ✓ → карточка поставщика ✓ → фильтры (/materials) ✓ — **V1.1 ЗАКРЫТ**.
**Следующее — не выбрано:** этап F/V2 (сравнение сезонов, графики Recharts, печатные формы) ИЛИ добор
V1.1-бэклога (фильтры дашбордов/справочников, PDF акта, аналитика брака, мобильная). Промпты — `docs/prompts/PROMPTS-*.md`.
