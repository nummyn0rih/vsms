...
V1.1 ЗАКРЫТ. Идёт **этап F/V2**: аналитический дашборд (Recharts) ✓, факт. средний вес рейса ✓. **Мобильная
версия v1** (responsive) — ЗАКРЫТА. **Мобильный v2** (read-only обзор, `getPlanWeek`): Сводка ✓ → **План (текущий,
последний v2)**. Далее: печатные формы (PDF) · сравнение сезонов (после истории, этап G) · бэклог V1.1-фильтров.
...
- [~] mobile-5: мобильный План (drawer «Планировщик» RO, карточки прогресса план/факт/%/Δ + дневная разбивка) — ТЕКУЩИЙ (последний v2). `PROMPTS-MOBILE-5-PLAN.md`.
