...
версия v1** (responsive) — ЗАКРЫТА. **Мобильный v2** (Сводка + План) — ЗАКРЫТ. **Печатные формы** (5 листов) — ЗАКРЫТЫ. **Следующий срез — не выбран:**
см. «Бэклог пользователя» ниже (печать всей ленты · развернуть все недели · фильтры в приёмке · палитра цветов ·
экспорт Excel · аналитика по культурам · календарь). Промпты — `docs/prompts/PROMPTS-*.md`.
...
- [~] **Аналитика по отдельным культурам** (drill-down в дашборде) — ТЕКУЩИЙ. Дизайн-спека `DESIGN-CULTURE-ANALYTICS.md`.
- [ ] **Аналитика по отдельным культурам** (drill-down в дашборде).
