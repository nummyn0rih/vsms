...
**Текущий фокус:** Этап B (Ядро отгрузок). Закрыты: B-контракты-мини, B1, B2, B2.5, **B3** (срезы 17a/17b + 17c тулбар Часть A/B — фильтры/поиск/тумблер/сброс/краевые состояния). Осталось по отгрузкам: заменить временные `AlertDialog` статусов на свёрстанные диалоги (`prototypes/status-dialogs-*`). Дальше — B4 (WeeklyPlan + прогресс).
