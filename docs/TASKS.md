...
версия v1** (responsive): шелл+лента ✓, приёмка ✓ → **контакты водителей (текущий, последний)**. Далее F/V2:
печатные формы (PDF) · сравнение сезонов (после истории, этап G). Промпты — `docs/prompts/PROMPTS-*.md`.
...
- [~] mobile-3: контакты водителей на мобиле (карточки + tel:) — ТЕКУЩИЙ (последний мобильный срез). `PROMPTS-MOBILE-3-DRIVERS.md`.
