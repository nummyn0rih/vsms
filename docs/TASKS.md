...
**Текущий фокус:** Этап B (Ядро отгрузок). B-контракты-мини закрыт. Следующий — B0 (дизайн-сессия) / B1 (server-ядро отгрузок).
...
- [ ] **B1 — server-ядро отгрузок:** Shipment + ShipmentItem (мультипозиции, привязка водителя), даты: ввод одной → вторая ±2 (BR-12), валидация рабочего дня по SeasonConfig (BR-11), привязки `contract_line_id`.
- [ ] **B2 — статусы + тара:** переходы `planned → sent` (BR-13); авто-движение тары на `sent` + сторно при откате `sent → planned` (BR-3, BR-19, StockMovement).
- [ ] **B3 — Лента:** ISO-недельная группировка, сворачивание, центрирование на текущей неделе, шапка недели, дневные подытоги.
