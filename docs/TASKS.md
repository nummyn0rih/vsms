...
**Текущий фокус:** Этап B — закрыт. Этап C (полная приёмка) в работе: **C0 + C1 + C2 — закрыты**. C2: движение ингредиента `consumption` на приёмке (факт×рецептура, `from=фермер · to=null`, привязка к акту, сторно нетто-паттерном, идемпотентно); миграция `c2_ingredient_consumption` (`MovementType += consumption`, `SourceDocType += acceptance_act`, `StockMovement.quantity → Decimal(15,6)`). Следующий срез — **C3 (деньги): C3a (расчёт стоимости/выполнения, backend) → C3b (дизайн зоны 3) → C3c (зона 3 код + откат) → C3d (выполнение/стоимость в Контрактах)**. Карта — `docs/ROADMAP-C.md`. B5 (доска dnd) — после C.
...
- [ ] **C3 — деньги (живой пересчёт, БР-1/6; принятый × цена строки):**
  - [ ] **C3a** — расчёт стоимости + выполнения строк (backend, чистые функции `server/contracts/execution.ts` + загрузчик `getContractExecution`; разнесение по привязкам категорий калибра `CalibreResult.contract_line_id`). БЕЗ UI. Живой пересчёт, ничего не хранится.
  - [ ] **C3b** — дизайн зоны 3 «Принято» (Claude Design, `docs/prototypes/acceptance-zone3-c3.html`).
  - [ ] **C3c** — зона 3 «Принято» код: просмотр принятых машин/актов (факт · к оплате · стоимость · № акта · строка) + откат акта (admin, переиспользует `revertAct` со сторно C2).
  - [ ] **C3d** — выполнение/стоимость по строкам в экране «Контракты» (на `getContractExecution`); опц. ручная привязка категории нестандарта к строке.
