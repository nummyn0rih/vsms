...
**Текущий фокус:** Этап B — закрыт. Chore (удаление осиротевших `previewShipmentTare`/`ShipmentTarePreview`) — **сделано**. Идём в **этап C — полная приёмка** (акт на позицию, % брака/калибры, принятый вес, движение ингредиентов, стоимость): срезы C0 (дизайн формы акта) → C1 → C2 → C3 → C4. Карта — `docs/ROADMAP-C.md`. B5 (доска dnd) — после C.
...
- [x] **C1a — форма акта приёмки (simple):** диалог «Акт» на позиции (`AcceptanceActDialog`) — № акта (BR-9, unique), фактический вес (через `setActualWeight`, BR-25), % брака → принятый вес `actual×(1−brak%)` (производное, не хранится), привязка строки контракта (BR-8, авто при одной/выбор при многих). `server/acceptance/act.ts`: `getActContext`/`saveAct`/`revertAct` (ActionResult, requireRole, $transaction+logChange). Авто-`accepted` машины при приёмке последней позиции, производная «Частично принята» (BR-13). Откат акта — admin, `accepted→arrived`, идемпотентно. Шорткат из зоны 1: `sent→arrived` + баннер. RBAC operator/admin, user read-only. БЕЗ склада (C2)/стоимости (C3).
- [x] **C1b — калибр в форме акта + общие фиксы (BR-10):** ветка `acceptance_type=calibre` — брак% → «Чистый вес» (=годное) → таблица категорий из `CalibreScheme` (размерные/безразмерные, не хардкод), «% от чистого» + кг рядом, Σ%=100% (валидация), привязка категорий к строкам (принятые → дефолт строки позиции/обязательна; нестандарт → null, ручная привязка admin). Принятый = `actual×(1−brak%)×Σ(принятых%)` (производное); нестандарт «не в зачёт». `saveAct` ветвится по типу, создаёт `CalibreResult` (каскад удаления при откате). Общие фиксы формы: вес+%брака в один ряд, «Годное»→«Чистый вес», убрана подпись «порча %», строка контракта отдельным полем в simple. Фикс ремонтирования диалога (ключ `openSeq`).
