...
**acceptance auto-date** — ЗАКРЫТЫ. Контуры поставки/тары/ингредиентов/планирования/переноса/откатов
замкнуты. **Следующий срез:** широкий V1.1 (Сводка/Heatmap наполнение, AlertRule-алерты, карточка
поставщика, фильтры) — нарезать на проверяемые единицы. Промпты — `docs/prompts/PROMPTS-*.md`.
