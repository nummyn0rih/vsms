...
**Текущий фокус:** Этап B (Ядро отгрузок). Закрыты: B-контракты-мини, B1, B2, B2.5, **B3 (лента: 17a/17b/fix-3/17c + диалоги статусов)**. Хвост: отдельный chore-коммит — удалить осиротевшие `previewShipmentTare`/`ShipmentTarePreview`. Следующий срез — **B4 (WeeklyPlan + прогресс к плану недели)**.
...
- [~] **B4 — WeeklyPlan + прогресс:** ввод недельного плана от завода, прогресс к плану недели. _(**B4a Часть A — код готов, миграция к применению при доступной БД.** Схема: `WeeklyPlan` += `season_year`, `date Date?`, два partial-unique индекса через preview-фичу `partialIndexes` (Prisma 7.4+, `where: raw(...)`) — заменили старый `@@unique(year,week,culture)`; `actual_weight_kg` уже в схеме. Server `server/plan/`: `schema.ts` (zod+типы сетки), `board.ts` `getPlanWeek` (чистый, культуры×рабочие дни+неделя, режим из наличия строк), `actions.ts` — `upsertPlanTarget`/`deletePlanTarget` (BR-20 валидация гранулярности), `convertDaysToWeek`/`convertWeekToDays` (BR-21 атомарно `$transaction`+`logChange`, распределение в милли-тоннах, остаток на последний рабочий день), `loadPlanWeek` (read для всех). UI: «План» активен в `.seg` тулбара (Heatmap disabled), `viewMode`+`planWeek` в `ShipmentsFeed` (своя неделя, ±7 дней через ISO-хелперы, независимо от feed.weeks), `PlanView` (матрица `.pmatrix` по `plan-view-b4.html`: sticky шапка/первый столбец, тумблер `.gran` с тултипами, `PlanInput` автосейв по blur + ремоунт по `version`, день-режим «Неделя»=Σ read-only, tfoot «Итого»), RBAC admin (UI + серверный requireRole). lint+tsc чисто. **Осталось:** `npm run db:migrate` (Neon был недоступен), затем ручной прогон ревью-чеклиста Части A. **Часть B готова:** `getPlanWeek` расширен агрегацией прогресса (BR-22, эффективный = actual ?? planned, раздельно actual/planRemaining/effective по день×культура и неделя×культура; суммы из неудалённых отгрузок всех статусов по `arrival_date`, граммы без float-погрешности, kg→т; `CellProgress` в `schema.ts`, `dayProgress`/`weekProgress` в строке, `dayTotalsProgress`/`weekTotalProgress` в неделе). `PlanView`: чистая `barGeometry`+`PlanBar` (факт сплошной / план штрих / перелёт ink за риской, риска-цель, `met`), подписи `DayCaption`/`WeekCaption` («эффективный / цель», бейдж дельты `+Δ`/`−Δ`/«факт 0»), tfoot-бар+итоги, `.ag` «набрано». Пустые: no-target ячейки — бар без риски, шкала к max эффективного по строке, подпись «нет цели»; совсем пусто — хинт-баннер над сеткой. Токены/стили в `globals.css` (`--bar-*`, `.pbar`/`.wcap`/`.wdelta`/`.nt-hint`/`.plan-hint`). tsc+lint+build чисто; агрегация сверена с БД (Σдней ≤ неделя, Σстрок = grand = raw 45,5 т). B4b (actual_weight UI/приёмка) — следующий срез. **B4c — состав культур недели (BR-23) — код готов, миграция к применению при доступной БД.** Схема: новая `WeeklyPlanScope { season_year, iso_year, iso_week, culture_id }`, `@@unique(iso_year,iso_week,culture_id)` + `@@index(iso_year,iso_week)`, обратная связь `Culture.weeklyPlanScopes`. Server: `actions.ts` += `addCultureToScope` (идемпотентно, upsert-семантика read-then-write) / `removeCultureFromScope` (идемпотентно) — `requireRole("admin")`+zod+`logChange` (`scope_add`/`scope_remove`); `board.ts` `getPlanWeek` фильтрует `rows` по видимости (scope ∨ цель ∨ отгрузки) и собирает `scopePicker` (все активные культуры с `lockReason`/`inScope`/`selected`/`locked`); расчёты прогресса/подытогов/headline BR-22 не тронуты. UI: загрузка вынесена в хук `usePlanWeek` (общая для матрицы и тулбара), `ScopeCombo` «Культуры · N» в `FeedToolbar` (slot `scopeSlot`, по `plan-scope-b4c.html`: поиск, секции locked/free, чекбоксы, причины, клик по locked — shake, read-only — тултип «состав задаёт администратор»), `PlanView` принимает неделю пропсами + пустая неделя «+ Добавить культуры». Стили scope в `globals.css`. tsc+lint чисто. **Осталось:** `npm run db:migrate` (Neon недоступен), затем прогон ревью-чеклиста B4c.)_
