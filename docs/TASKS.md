...
**Текущий фокус:** Этап B — закрыт. Этап C (полная приёмка) в работе: **C0 (дизайн формы акта) + C1 (форма акта simple+calibre, приёмка позиции) — закрыты** (вкл. fix/fix-2: одноступенчатая калибр-модель, лента по позиции). Следующий срез — **C2 (движение ингредиентов на приёмке позиции)** → C3 (стоимость + выполнение строк + зона 3). Карта — `docs/ROADMAP-C.md`. B5 (доска dnd) — после C.
...
Осталось в C:
- [ ] **C2** — движение ингредиента на приёмке позиции (единый `StockMovement kind=ingredient`, фактический вес, фермер→завод), сторно при откате акта, идемпотентность.
- [ ] **C3** — стоимость (принятый × цена строки) + выполнение строк контракта (живой пересчёт, привязки категорий, BR-6) + зона 3 «Принято» (просмотр актов, откат admin).
