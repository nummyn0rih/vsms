...
**Текущий фокус:** Этапы A, B, C, D — закрыты. **Этап E (ингредиенты/специи, паритет с тарой) — ЗАКРЫТ** (E1–E4): доставка ингредиента завод→фермер тем же `MaterialShipment` (параметризация по `item_kind`, смешанный груз штатно), начальные остатки ингредиентов (`opening`), дашборд балансов `/ingredients` (транзит `-2`, кг/л раздельно, «Итого» учитывает расход). Миграции не потребовалось. Реальные остатки ингредиентов у завода и фермеров видны. **Следующий выбор:** D2-ops (ручной лом/корректировка) · D3-2 (позиционное прибытие рейса) · farmer→farmer transfer (правка схемы) · B5 (доска dnd) · широкий V1.1 (потребность/алерты, heatmap, карточка поставщика, фильтры). Промпты этапа E — `docs/PROMPTS-E*.md`.
...
- [ ] **D2-ops** (опц.) — ручные операции тары: лом (`scrap`), корректировка (`adjustment`), утиль (`disposal`). Пока scrap всегда 0.
- [ ] **D3-2** (опц.) — позиционное прибытие рейса (`MaterialShipmentItem += arrived_at`, производный статус машины). Переиспользуется в E.
