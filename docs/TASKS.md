...
**Текущий фокус:** Этапы A, B, C — закрыты. **Этап D (жизненный цикл тары) — ЗАКРЫТ** (D1–D4): начальные остатки (`opening`), двухфазное движение тары через транзит-сентинелы (`-1` на завод, `-2` с завода), доставка тары завод→фермер (`MaterialShipment`), дашборд реальных остатков (матрица локация×тип + транзит). Реальные остатки тары у завода и фермеров видны. **Следующий выбор:** этап E (ингредиенты/специи: доставка + балансы, паритет с тарой; +transfer фермер→фермер) ИЛИ D2-ops (ручной лом/корректировка) ИЛИ D3-2 (позиционное прибытие рейса) ИЛИ B5 (доска dnd). Промпты этапа D — `docs/PROMPTS-D*.md`. Опц. хвосты C: C3d-2/C3d-3 — по нужде.
